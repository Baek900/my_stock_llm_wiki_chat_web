...
start "Agent-Guru Backend Server" cmd /c "cd backend && :loop && echo Starting Backend... && ..\.venv\Scripts\python.exe -m uvicorn main:app --host 127.0.0.1 --port 8080 --reload && echo Backend stopped. Restarting... && goto loop"
...
:: 3. Start Lemonade Server in a new window with auto-restart loop
echo [LEMONADE] Starting Lemonade Server (Port 8000) in a new terminal...
start "Agent-Guru Lemonade Server" cmd /c "cd /d C:\Users\qorrb\AppData\Local\lemonade_server\bin && :loop && echo Starting Lemonade Server... && LemonadeServer.exe && echo Lemonade Server stopped. Restarting... && goto loop"
...
echo  - Local LLM: http://localhost:8000
echo.
